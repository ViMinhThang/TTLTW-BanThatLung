                            ...
                            <div class="content-padding">
                                <div class="description">
                                    <div class="text-content">
                                        <p class="gl-vspace">${belt.beltVariant.description}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                                                ...
                                                <div class="row mt-2">
                                                    <div class="d-flex align-item-center">
                                                        ...
                                                        <div class="ms-4">
                                                            ...
                                                            <textarea name="desc" class="pt-2 ps-1 mt-4" cols="70"
                                                                      rows="5"
                                                                      style="resize: none">
                                                    </textarea>
                                                        </div>
                    ...
                    <c:forEach var="variant" items="${similarVariants}">
                        <a href="${pageContext.request.contextPath}/productDetails?beltId=${param.beltId}&variantId=${variant.id}"
                           ...
                           style="flex-direction: column; width: 100px">
                            <img class="img-fluid" src="${pageContext.request.contextPath}${variant.images[0]}"/>
            ...
            <div class="mb-3 mt-2">
                ...
                <div class="d-flex gap-2">
                    ...
                    <c:forEach var="size" items="${fn:split(sizes, ',')}">
                        <c:set var="matchedVariantId" value=""/>
                        <c:forEach var="variant" items="${similarVariants}">
                            <c:if test="${variant.size eq size}">
                                <c:set var="matchedVariantId" value="${variant.id}"/>
                            </c:if>
                        </c:forEach>

                        <a href="${not empty matchedVariantId ? pageContext.request.contextPath.concat('/productDetails?beltId=').concat(param.beltId).concat('&variantId=').concat(matchedVariantId) : '#'}"
                           class="variantTag text-black d-inline-flex justify-content-center align-items-center p-2"
                           style="background-color: ${empty matchedVariantId ? 'lightgray' : 'darkgray'}; opacity: ${empty matchedVariantId ? '0.5' : '1'}; pointer-events: ${empty matchedVariantId ? 'none' : 'auto'}; border-radius: 0; min-width: 50px;">
                            <p class="m-0">${size}</p>
                        </a>
                    </c:forEach>
                </div>
            ...
            <div class="mb-3 d-flex flex-column align-item-center">
                <form method="POST" action="/buyNow">
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <input type="hidden" name="price" value="${belt.beltVariant.price}">
                        </c:otherwise>
                    </c:choose>
