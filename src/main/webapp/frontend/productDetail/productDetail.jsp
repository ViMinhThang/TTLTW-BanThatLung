...
<div class="my-5 bg-white rounded mb-0 ps-3 py-2 pe-4">
    <div class="d-flex">
        <div class="col-md-9 d-flex">
            <%-- Lấy danh sách biến thể có cùng màu --%>
            <div class="col-2 belts-thumbnails d-flex flex-column align-items-start justify-content-start mt-0 me-2">
                <c:forEach var="variant" items="${SimilarVariantColor}">
                    <!-- Only the first image of each variant -->
                    <c:if test="${not empty variant.images}">
                        <img src="${pageContext.request.contextPath}${variant.images[0]}"
                             class="mb-2 w-50"
                             alt="Thumbnail 1" data-bs-target="#productCarousel"
                             data-bs-slide-to="0"/>
                    </c:if>
                </c:forEach>
            </div>

            <%-- Carousel để hiển thị ảnh lớn --%>
                                    ...
                                    <div class="d-flex flex-column mt-5 custom_bg p-4">
                                        <form action="${pageContext.request.contextPath}/productDetails" method="POST">
                                            ...
                                            <div class="d-flex justify-content-end">
                                                <c:choose>
                                                    <c:when test="${sessionScope.auth != null}">
                                                        <button type="submit" class="btn btn-dark me-4 p-2 px-3 mt-3">
                                                            Gửi
                                                            đánh
                                                            giá
                                                        </button>
