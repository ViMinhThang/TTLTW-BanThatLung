$(document).ready(function () {
    let dropdown = $(".search-dropdown");
    let productList = $("#list__product__row");
    dropdown.removeClass("show");

    function showDropdown(query) {
        productList.empty();

        if (query.length > 0) {
            dropdown.addClass("show");
            $.ajax({
                url: `/search?q=${query}`,
                method: "GET",
                success: function (data) {
                    productList.empty();

                    if (data.length > 0) {
                        data.forEach(belt => {
                            const variant = belt.beltVariant;
                            if (variant) {
                                const isOutOfStock = variant.stockQuantity <= 0;

                                productList.append(`
                <div class="col-2">
                    ${isOutOfStock ? `
                        <div class="text-center hover--black position-relative">
                            <div class="position-absolute top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center"
                                 style="z-index: 2;">
                                <div style="background-color: rgba(0, 0, 0, 0.7); color: white; width: 200px; height: 200px;
                                            border-radius: 50%; display: flex; align-items: center; justify-content: center;
                                            font-size: 20px; font-weight: bold;">
                                    Hết hàng
                                </div>
                            </div>
                            <img src="${variant.images?.[0] || '/default-image.jpg'}"
                                 class="img-fluid w-100 shadow-sm"
                                 alt="${belt.name}" style="height: 10rem; object-fit: cover; opacity: 0.3;" />
                            <div class="mt-2 text-start ps-3 pb-3">
                                <p class="fw-bold fs-5 mb-1">${variant.price.toLocaleString()} VNĐ</p>
                                <h4 class="text-muted mb-1">${belt.name}</h4>
                                <p class="badge bg-secondary">${belt.discountRate}%</p>
                            </div>
                        </div>
                    ` : `
                        <a href="/productDetails?beltId=${belt.id}&variantId=${variant.id}" 
                           class="text-decoration-none text-dark">
                            <div class="text-center hover--black border border-white">
                                <img src="${variant.images?.[0] || '/default-image.jpg'}"
                                     class="img-fluid w-100 rounded shadow-sm"
                                     alt="${belt.name}" style="height: 10rem; object-fit: cover;" />
                                <div class="mt-2 text-start ps-3 pb-3">
                                    <p class="fw-bold fs-5 mb-1">${variant.price.toLocaleString()} VNĐ</p>
                                    <h4 class="text-muted mb-1">${belt.name}</h4>
                                    <p class="badge bg-secondary">${belt.discountRate}%</p>
                                </div>
                            </div>
                        </a>
                    `}
                </div>
            `);
                            }
                        });
                    } else {
                        productList.append(`<div class="text-center text-muted">Không tìm thấy sản phẩm</div>`);
                    }

                },
                error: function () {
                    productList.append(`<div class="text-danger">Lỗi khi lấy dữ liệu</div>`);
                }
            });
        }
    }

    $(".search__input").on("input click", function () {
        showDropdown($(this).val().trim());
        $(".dimmer").addClass("active");
        $(".search_container").css("z-index", "1");
    });

    $(document).on("click", function (e) {
        if (!$(e.target).closest(".search__input, .search-dropdown").length) {
            dropdown.removeClass("show");
            $(".dimmer").removeClass("active");
        }
    });
});
